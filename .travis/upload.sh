if [ -f /tmp/os-loader-builddir/IMAGE/output/image.iso ]; then
  if [ "${TRAVIS_PULL_REQUEST}" = "false" ]; then
    ...
    doupload="curl -X POST -F iso=@/tmp/os-loader-builddir/IMAGE/output/image.iso -F key=$UPLOADKEY -F ap=$ap https://mkg20001.sytes.net/os-loader/iso.php --connect-timeout 60 -m 300"
    $doupload
    if [ $? -ne 0 ]; then
